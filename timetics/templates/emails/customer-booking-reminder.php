...
defined( 'ABSPATH' ) || exit;
    ...
    $timetics_email_body  = timetics_get_option( 'booking_reminder_customer_email_body' );
    $timetics_email_title = timetics_get_option( 'booking_reminder_customer_email_title' );
    ...
    <div style="max-width: 600px; margin: 0 auto;">
        <div class="email-content" style="padding: 40px; background-color: #ffffff; border-radius: 0 0 12px 12px; border-top: 4px solid #3161F1; margin-bottom: 40px;">
            ...
            <?php else: ?>
                ...
                <p style="color: #556880; margin-bottom: 5px"><?php
                    printf(
                        /* translators: %1$s: Booking date, %2$s: Booking time, %3$s: Meeting name */
                        esc_html__( 'I hope this email finds you well. This is just a friendly reminder that you have a reservation with us on %1$s at %2$s. We are looking forward to welcoming you to %3$s.', 'timetics' ),
                        esc_html( $timetics_date ),
                        esc_html( $timetics_time ),
                        esc_html( $this->meeting->name )
                    );
                ?></p>
